...
import db_utils, random_graphs, config
...
def test_TiP_network_closeness(TiPmatrix,TiP_cutoff,closeness_types,CSNs,network_name,num_rand):
	...
	for closeness_type in closeness_types:
		...
		target.write('condition\treal_obs\tnum_rand\tnum_rand_obs_greater\tnum_rand_obs_smaller\t' + 					'mean_rand_obs\tstd_rand_obs\tmedian_rand_obs\t5th_perc_rand_obs\t95_perc_rand_obs\t' + 					'z_score\tp_value_greater\tp_value_smaller\tnum_TiP_genes_in_CSN\tnum_PPI_TiP_genes_in_CSN\n')
	...
	for c,condition in enumerate(conditions):
		...
		for t,closeness_type in enumerate(closeness_types):
			...
			if real_obs_list[t] == 'NaN' or num_valid < 100:
				targets[t].write(condition + '\tNaN\t' + str(num_rand) + '\tNaN\tNaN\tNaN\tNaN\tNaN\tNaN\tNaN\tNaN\tNaN\tNaN\t' + 								 (str(len(TiP_genes_in_LCC_CSN)) if condition in ['asp','lcc'] else str(len(TiP_genes_in_CSN))) + '\t' + 								 (str(len(get_num_interaction_partners(LCC_CSN,TiP_genes_in_LCC_CSN))) if 								 condition in ['asp','lcc'] else str(len(get_num_interaction_partners(CSN,TiP_genes_in_CSN)))) + '\n')
			else:
				...
				targets[t].write(condition + '\t' + str(real_obs_list[t]) + '\t' + str(num_valid) + '\t' + str(len(greater_rand_obs)) + '\t' + 							str(len(smaller_rand_obs)) + '\t' + str(mean_rand_obs) + '\t' + str(std_rand_obs) + '\t' + 							str(median_rand_obs) + '\t' + str(perc5_rand_obs) + '\t' + str(perc95_rand_obs) + '\t' + 							str(z_score) + '\t' + str(p_value_g) + '\t' + str(p_value_s) + '\t' + 							 (str(len(TiP_genes_in_LCC_CSN)) if condition in ['asp','lcc'] else str(len(TiP_genes_in_CSN))) + '\t' + 							 (str(len(get_num_interaction_partners(LCC_CSN,TiP_genes_in_LCC_CSN))) if condition in ['asp','lcc'] else str(len(get_num_interaction_partners(CSN,TiP_genes_in_CSN)))) + '\n')
